...
async function saveMoves(moves) {
    const db = await getDB();
    await db.put("moves", moves, "moves");
}
async function loadMoves() {
    const db = await getDB();
    return (await db.get("moves", "moves")) || [];
}
...
function useLocale() {
    ...
    useEffect(() => {
        const h = () => setLocale(localStorage.getItem("locale") || "en");
...
function generateICS(events, { calendarName = "ChemoCare" } = {}) {
    const t = STRINGS[localStorage.getItem("locale") || "en"];
    const lines = [
        "BEGIN:VCALENDAR",
        "VERSION:2.0",
        "PRODID:-//ChemoCare//EN",
...
/******************** UI Components ********************/
function OffsetInfo({ t }) {
    return (_jsxs(Popover, { children: [_jsx(PopoverTrigger, { asChild: true, children: _jsx("button", { "aria-label": t.offsetHelpBtn, className: "inline-flex items-center justify-center align-middle w-5 h-5 rounded-full border text-[10px] leading-none ml-1 hover:bg-gray-100", title: t.offsetHelpTitle, children: "?" }) }), _jsxs(PopoverContent, { className: "w-64 text-sm", children: [_jsx("div", { dangerouslySetInnerHTML: { __html: t.offsetExpl } }), _jsx("ul", { className: "list-disc ml-4 mt-2 space-y-1", dangerouslySetInnerHTML: { __html: t.offsetM1 + t.offset0 + t.offsetP1 } })] })] }));
}
function Header({ onReset }) {
    const { locale, t, changeLocale } = useLocale();
    return (_jsxs("div", { className: "w-full flex items-center justify-between mb-4", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx(CalendarIcon, { className: "w-6 h-6" }), _jsx("h1", { className: "text-2xl font-bold", children: t.appName })] }), _jsxs("div", { className: "flex gap-2 items-center", children: [_jsxs(Dialog, { children: [_jsx(DialogTrigger, { asChild: true, children: _jsxs(Button, { variant: "outline", children: [_jsx(Share2, { className: "w-4 h-4 mr-2" }), t.shareExport] }) }), _jsxs(DialogContent, { children: [_jsx(DialogHeader, { children: _jsx(DialogTitle, { children: t.shareTitle }) }), _jsx(SharePanel, {})] })] }), _jsxs(Select, { value: locale, onValueChange: (v) => changeLocale(v), children: [_jsx(SelectTrigger, { className: "w-[120px]", children: _jsx(SelectValue, { placeholder: t.language }) }), _jsxs(SelectContent, { children: [_jsx(SelectItem, { value: "en", children: t.english }), _jsx(SelectItem, { value: "nl", children: t.dutch })] })] }), _jsxs(Button, { variant: "secondary", onClick: onReset, children: [_jsx(Trash2, { className: "w-4 h-4 mr-2" }), t.reset] })] })] }));
}
function SetupWizard({ onComplete }) {
    ...
    const updateRule = (id, patch) => setMedRules(r => r.map(x => x.id === id ? { ...x, ...patch } : x));
    const deleteRule = (id) => setMedRules(r => r.filter(x => x.id !== id));
    const canSave = startDate && frequencyDays >= 1;
    return (_jsx(Card, { className: "max-w-3xl mx-auto", children: _jsxs(CardContent, { className: "p-6", children: [_jsx("h2", { className: "text-xl font-semibold mb-2", children: t.setupTitle }), _jsx("p", { className: "text-sm opacity-80 mb-4", dangerouslySetInnerHTML: { __html: t.setupHelp } }), _jsxs("div", { className: "grid md:grid-cols-3 gap-4 mb-6", children: [_jsxs("div", { children: [_jsx(Label, { children: t.firstDate }), _jsx(Input, { type: "date", value: startDate, onChange: e => setStartDate(e.target.value) })] }), _jsxs("div", { children: [_jsx(Label, { children: t.frequency }), _jsx(Input, { type: "number", step: 1, min: 1, value: frequencyDays, onChange: e => setFrequencyDays(parseInt(e.target.value || "0", 10)) })] }), _jsxs("div", { children: [_jsx(Label, { children: t.cycles }), _jsx(Input, { type: "number", step: 1, min: 1, value: cycles, onChange: e => setCycles(e.target.value === "" ? "" : parseInt(e.target.value, 10)) })] })] }), _jsx("h3", { className: "font-medium mb-2", children: t.medActions }), _jsx("div", { className: "space-y-2 mb-4", children: medRules.map(rule => (_jsxs("div", { className: "grid md:grid-cols-12 items-center gap-2 p-2 rounded-lg border", children: [_jsxs("div", { className: "md:col-span-2", children: [_jsxs("div", { className: "flex items-center gap-1", children: [_jsx(Label, { className: "text-xs", children: t.offsetDays }), _jsx(OffsetInfo, { t: t })] }), _jsx(Input, { type: "number", step: 1, value: rule.offset, onChange: e => updateRule(rule.id, { offset: parseInt(e.target.value || "0", 10) }) })] }), _jsxs("div", { className: "md:col-span-3", children: [_jsx(Label, { className: "text-xs", children: t.title }), _jsx(Input, { value: rule.title, onChange: e => updateRule(rule.id, { title: e.target.value }) })] }), _jsxs("div", { className: "md:col-span-6", children: [_jsx(Label, { className: "text-xs", children: t.notesOpt }), _jsx(Input, { value: rule.notes, onChange: e => updateRule(rule.id, { notes: e.target.value }) })] }), _jsxs("div", { className: "md:col-span-1 flex items-center gap-2", children: [_jsx(Checkbox, { checked: rule.enabled, onCheckedChange: v => updateRule(rule.id, { enabled: !!v }) }), _jsx(Button, { size: "icon", variant: "ghost", onClick: () => deleteRule(rule.id), children: _jsx(Trash2, { className: "w-4 h-4" }) })] })] }, rule.id))) }), _jsx("div", { className: "flex gap-2 mb-6", children: _jsxs(Button, { variant: "outline", onClick: addRule, children: [_jsx(Plus, { className: "w-4 h-4 mr-2" }), t.addAction] }) }), _jsx("div", { className: "flex justify-end", children: _jsx(Button, { disabled: !canSave, onClick: async () => {
                            const settings = {
                                startDate: startDate,
...
function PlanEditor({ settings, onSaved }) {
    ...
    const updateRule = (id, patch) => { setVals((v) => ({ ...v, medRules: v.medRules.map((r) => r.id === id ? { ...r, ...patch } : r) })); setDirty(true); };
    ...
    return (_jsxs("div", { className: "space-y-4", children: [_jsxs("div", { className: "grid md:grid-cols-3 gap-4", children: [_jsxs("div", { children: [_jsx(Label, { children: t.firstDate }), _jsx(Input, { type: "date", value: vals.startDate, onChange: e => update({ startDate: e.target.value }) })] }), _jsxs("div", { children: [_jsx(Label, { children: t.frequency }), _jsx(Input, { type: "number", step: 1, min: 1, value: vals.frequencyDays, onChange: e => update({ frequencyDays: parseInt(e.target.value || "0", 10) }) })] }), _jsxs("div", { children: [_jsx(Label, { children: t.cycles }), _jsx(Input, { type: "number", step: 1, min: 1, value: vals.cycles ?? "", onChange: e => update({ cycles: e.target.value === "" ? null : parseInt(e.target.value, 10) }) })] })] }), _jsx("h4", { className: "font-medium", children: t.medActions }), _jsx("div", { className: "space-y-2", children: vals.medRules.map((rule) => (_jsxs("div", { className: "grid md:grid-cols-12 items-center gap-2 p-2 rounded-lg border", children: [_jsxs("div", { className: "md:col-span-2", children: [_jsxs("div", { className: "flex items-center gap-1", children: [_jsx(Label, { className: "text-xs", children: t.offsetDays }), _jsx(OffsetInfo, { t: t })] }), _jsx(Input, { type: "number", step: 1, value: rule.offset, onChange: e => updateRule(rule.id, { offset: parseInt(e.target.value || "0", 10) }) })] }), _jsxs("div", { className: "md:col-span-3", children: [_jsx(Label, { className: "text-xs", children: t.title }), _jsx(Input, { value: rule.title, onChange: e => updateRule(rule.id, { title: e.target.value }) })] }), _jsxs("div", { className: "md:col-span-6", children: [_jsx(Label, { className: "text-xs", children: t.notes }), _jsx(Input, { value: rule.notes, onChange: e => updateRule(rule.id, { notes: e.target.value }) })] }), _jsxs("div", { className: "md:col-span-1 flex items-center gap-2", children: [_jsx(Checkbox, { checked: rule.enabled, onCheckedChange: v => updateRule(rule.id, { enabled: !!v }) }), _jsx(Button, { size: "icon", variant: "ghost", onClick: () => deleteRule(rule.id), children: _jsx(Trash2, { className: "w-4 h-4" }) })] })] }, rule.id))) }), _jsxs("div", { className: "flex gap-2", children: [_jsxs(Button, { variant: "outline", onClick: () => addRule(), children: [_jsx(Plus, { className: "w-4 h-4 mr-2" }), t.addAction] }), _jsx("div", { className: "flex-1" }), _jsx(Button, { disabled: !dirty, onClick: async () => {
                            ...
                            onSaved();
                        }, children: t.savePlan })] }), _jsx("p", { className: "text-xs opacity-70", dangerouslySetInnerHTML: { __html: t.editAffects } })] }));
}
...
function SharePanel() {
    ...
    return (_jsxs("div", { className: "space-y-3", children: [_jsx("p", { className: "text-sm opacity-80", dangerouslySetInnerHTML: { __html: t.shareBody } }), _jsx("div", { className: "flex gap-2", children: _jsxs(Button, { onClick: () => {
                    ...
                    }, children: [_jsx(Download, { className: "w-4 h-4 mr-2" }), t.exportICS] }) }), _jsxs("details", { className: "rounded-lg border p-3", children: [_jsx("summary", { className: "cursor-pointer font-medium", children: t.howToImport }), _jsx("ul", { className: "list-disc ml-6 text-sm mt-2 space-y-1", dangerouslySetInnerHTML: { __html: t.outlookDesktop + t.outlookWeb + t.googleCal + t.appleCal } }), _jsx("p", { className: "text-xs opacity-70 mt-2", children: t.subNote })] })] }));
}
function MonthCalendar({ events, onMoveTreatment }) {
    ...
    return (_jsxs("div", { className: "space-y-3", children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsx(Button, { variant: "outline", onClick: () => setCursor(new Date(cursor.getFullYear(), cursor.getMonth() - 1, 1)), children: t.prev }), _jsx("div", { className: "text-lg font-semibold", children: cursor.toLocaleDateString(undefined, { month: "long", year: "numeric" }) }), _jsx(Button, { variant: "outline", onClick: () => setCursor(new Date(cursor.getFullYear(), cursor.getMonth() + 1, 1)), children: t.next })] }), _jsxs("div", { className: "grid grid-cols-7 gap-1", children: [t.weekdays.map((d) => _jsx("div", { className: "text-xs text-center opacity-70 py-1", children: d }, d)), days.map((day, i) => {
                        ...
                        return (_jsxs("div", { className: `min-h-[88px] p-2 rounded-xl border ${inMonth ? "bg-white" : "bg-gray-100 opacity-70"} ${isToday ? "ring-2 ring-blue-400" : ""}`, children: [_jsx("div", { className: "text-xs mb-1 opacity-70", children: day.getDate() }), _jsx("div", { className: "space-y-1", children: evs.map((ev) => (_jsxs("div", { className: `text-[11px] px-2 py-1 rounded-full ${ev.type === "treatment" ? "bg-green-100" : "bg-yellow-100"} flex items-center justify-between gap-1`, children: [_jsx("span", { className: "truncate", children: ev.type === 'treatment' ? `${t.treatment} #${ev.index + 1}` : ev.title }), ev.type === "treatment" && (_jsx("button", { className: "text-[10px] underline", onClick: () => onMoveTreatment(ev), children: t.moveEllipsis }))] }, ev.id))) })] }, i));
                    })] })] }));
}
function App() {
    const { t } = useLocale();
...
function runLightTests() {
    try {
        ...
        const firstPre = evs.find((e) => e.id === "act-0-pre");
        const firstPost = evs.find((e) => e.id === "act-0-post");
        const firstSame = evs.find((e) => e.id === "act-0-same");
        ...
        const nlT = STRINGS[localStorage.getItem("locale") || "en"];
